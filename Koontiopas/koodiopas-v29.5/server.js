...
async function copyTemplateFolderToRepo({templateOwner,templateRepo,folder,targetOwner,targetRepo,token}) {
  ...
  if(!files.length) throw new Error(`Template-repositorysta ei löytynyt tehtäväkansiota: ${folder}`);
...
async function publishAssignmentFolder({course,assignment,folder,targetOwner,targetRepo,token}) {
  // Jos kurssille on määritetty lähderepository ja siellä on saman niminen tehtäväkansio,
  // kopioidaan sen tiedostot. Muussa tapauksessa Koodiopas luo kansion itse.
  if(course.templateRepoUrl) {
    try {
      const {owner,repo}=parseGithubRepoUrl(course.templateRepoUrl);
      const files=await listTemplateFolderFiles(owner,repo,folder,token);
      if(files.length) {
        const count=await copyTemplateFolderToRepo({
          templateOwner:owner,templateRepo:repo,folder,
          targetOwner,targetRepo,token
        });
        return {mode:"template-folder",files:count};
      }
    } catch {
      // Template ei saa estää tehtävän julkaisua. Pudotaan generoituun kansioon.
    }
  }
...
async function ensureCourseRepo({course,student,org,token,isPrivate}) {
  ...
  if(existing.ok) return {repoName,repoUrl:url,created:false,creationMode:"existing"};

  // Jos kurssin repo on oikea GitHub Template repository, käytetään sitä yhteisten tiedostojen pohjana.
  if(course.templateRepoUrl) {
    try {
      const {owner,repo}=parseGithubRepoUrl(course.templateRepoUrl);
      ...
      if(info.is_template) {
        await ghJson(
          `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/generate`,
          {
            method:"POST",
            headers:{...ghHeaders(token),"Content-Type":"application/json"},
            body:JSON.stringify({
              owner:org,name:repoName,
              description:`${course.name} – ${student.name||student.id}`,
              private:Boolean(isPrivate),
        ...
        return {repoName,repoUrl:url,created:true,creationMode:"template"};
      }
    } catch {
      // Jos template on väärä tai käyttöoikeus puuttuu, yritetään silti luoda tavallinen kurssirepo.
    }
  }
...
app.post("/api/distribute/run", requireTeacher, async (req,res)=>{
  try{
    ...
    for(const student of group.students||[]) {
      ...
      try{
        // 1. Luo kurssirepo vain tarvittaessa.
        const repo=await ensureCourseRepo({
          course,student,org:cfg.githubOrg,token,isPrivate
        });

        // 2. Varmista opiskelijan push-oikeus myös jo olemassa olevaan repoon.
        const collaborator=await fetch(
          `https://api.github.com/repos/${encodeURIComponent(cfg.githubOrg)}/${encodeURIComponent(repo.repoName)}/collaborators/${encodeURIComponent(student.githubUsername)}`,
          {
            method:"PUT",
            headers:{...ghHeaders(token),"Content-Type":"application/json"},
        ...
        let inviteError="";
        if(!collaborator.ok) {
          try{ const d=await collaborator.json(); inviteError=d.message||""; }catch{}
        }

        // 3. Lisää juuri valittu tehtäväkansio, jos sitä ei ole.
        const folderAlreadyExists=await githubPathExists(
          cfg.githubOrg,repo.repoName,folder,token
        );
        ...
        if(!folderAlreadyExists) {
          publication=await publishAssignmentFolder({
            ...
            targetOwner:cfg.githubOrg,targetRepo:repo.repoName,token
          });
        }

        let status;
        if(!collaborator.ok) status="created_invite_failed";
        else if(folderAlreadyExists) status=repo.created?"course_repo_created_assignment_exists":"assignment_exists";
        else if(repo.created) status="course_repo_created_and_assignment_published";
        else status="assignment_published";
...
app.listen(PORT, () => {
  console.log(`Koodiopas v29.5: ${BASE_URL}`);
