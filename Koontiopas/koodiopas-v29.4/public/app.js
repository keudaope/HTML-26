...
$("preview").onclick = async () => {
  ...
  const d = await r.json();
  if (!r.ok) {
    if (d.code === "COURSE_TEMPLATE_REQUIRED") {
      $("missingTemplateCard").hidden = false;
      $("distributionTemplateUrl").value = "";
      $("distributionTemplateStatus").textContent =
        `Kurssi: ${d.course?.name || ""}`;
      return;
    }
...
function statusLabel(status) {
  return status === "course_repo_created" ? "✅ kurssirepo luotu" :
