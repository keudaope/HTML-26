...
app.post("/api/distribute/preview", requireTeacher, (req, res) => {
  try {
    ...
    const courseTemplate=course.templateRepoUrl || (course.assignments||[]).find(a=>a.templateRepoUrl)?.templateRepoUrl || "";
    if (!courseTemplate) {
      return res.status(409).json({
        error:"Kurssilta puuttuu Template repository.",
        code:"COURSE_TEMPLATE_REQUIRED",
        course:{id:course.id,name:course.name}
      });
    }
    course.templateRepoUrl=courseTemplate;
    parseGithubRepoUrl(course.templateRepoUrl);
    ...
    res.json({
      course:{id:course.id,name:course.name,templateRepoUrl:course.templateRepoUrl},
...
app.post("/api/distribute/run", requireTeacher, async (req,res)=>{
  try{
    ...
    const courseTemplate=course.templateRepoUrl || (course.assignments||[]).find(a=>a.templateRepoUrl)?.templateRepoUrl || "";
    if(!courseTemplate) return res.status(409).json({
      error:"Kurssilta puuttuu Template repository.",
      code:"COURSE_TEMPLATE_REQUIRED",
      course:{id:course.id,name:course.name}
    });
    ...
    const {owner:templateOwner,repo:templateRepo}=parseGithubRepoUrl(course.templateRepoUrl);
    ...
    if(!templateInfo.is_template) {
      return res.status(400).json({error:"Kurssin repository ei ole merkitty GitHubissa Template repositoryksi."});
    }
    ...
    for(const student of group.students||[]) {
      ...
      try{
        ...
        if(!repoResponse.ok) {
          // Ensimmäinen julkaisu kurssille: luo yksi repository kurssin templatesta.
          await ghJson(
          ...
          results.push({
            ...
            status:collaborator.ok?"course_repo_created":"created_invite_failed",
            error:inviteError
          });
          continue;
        }

        // Kurssirepo on jo olemassa. Lisää vain puuttuva tehtäväkansio.
        const exists=await githubPathExists(cfg.githubOrg,repoName,folder,token);
        if(exists) {
          results.push({
            student:student.name||student.id,
            githubUsername:student.githubUsername,
            repoName,repoUrl,folder,
            status:"assignment_exists"
          });
          continue;
        }

        const copied=await copyTemplateFolderToRepo({
          templateOwner,templateRepo,folder,
...
app.listen(PORT, () => {
  console.log(`Koodiopas v29.4: ${BASE_URL}`);
