...
async function getRawFile(owner, repo, path) {
  ...
  const response = await fetch(
    ...
    { headers: { "User-Agent": "Koodiopas-v7" } }
  );
...
app.post("/api/teacher/login", (req, res) => {
  ...
  if (!configured || configured === "vaihda_tahan_vahva_salasana") {
    return res.status(503).json({
      error: "Opettajan salasanaa ei ole vielä määritelty .env-tiedostoon."
    });
  }
...
app.listen(PORT, () => {
  console.log(`Koodiopas v7: http://localhost:${PORT}`);
});
