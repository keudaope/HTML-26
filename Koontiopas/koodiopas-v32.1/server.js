...
function migrateData() {
  ...
  for (const course of coursesData.courses || []) {
    ...
    if (!("repoPrefix" in course)) { course.repoPrefix = slug(course.name || "kurssi"); courseChanged = true; }
    if (!("templateRepoUrl" in course)) { course.templateRepoUrl = ""; courseChanged = true; }
    if (!("privateRepo" in course)) { course.privateRepo = true; courseChanged = true; }

    // V29-migraatio: vanhassa mallissa template oli tehtävällä.
    // Jos kurssilta puuttuu template, nosta ensimmäinen vanha tehtävä-template kurssitasolle.
    if (!course.templateRepoUrl) {
      const legacyTemplate=(course.assignments||[]).find(a=>a.templateRepoUrl)?.templateRepoUrl || "";
      if (legacyTemplate) {
        course.templateRepoUrl = legacyTemplate;
        courseChanged = true;
      }
    }
    for (const assignment of course.assignments) {
      const defaults = {
        ...
        dueAt: "",
        folder: `tehtava-${String((course.assignments||[]).indexOf(assignment)+1).padStart(2,"0")}-${slug(assignment.name||"tehtava")}`,
        published: true,
        module: ""
      };
      for (const [key, value] of Object.entries(defaults)) {
    ...
    courses: courses
      ...
      .map(course => ({
        ...
        assignments: (course.assignments || []).map(assignment => ({
          ...assignment,
          repoUrl: courseRepoUrl(course, match.student),
          folder: assignmentFolder(course, assignment)
        }))
      }))
  };
}
...
function courseRepoName(course, student) {
  const prefix=course.repoPrefix || slug(course.name || "kurssi");
  return `${prefix}-${slug(student.githubUsername || student.id || student.name)}`;
}
function courseRepoUrl(course, student) {
  const org=settings().githubOrg;
  return org ? `https://github.com/${org}/${courseRepoName(course,student)}` : "";
}
function assignmentFolder(course, assignment) {
  const index=Math.max(0,(course.assignments||[]).findIndex(a=>a.id===assignment.id));
  return assignment.folder || `tehtava-${String(index+1).padStart(2,"0")}-${slug(assignment.name||"tehtava")}`;
}

async function githubPathExists(owner, repo, pathName, token) {
  const r=await fetch(
    `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${pathName.split("/").map(encodeURIComponent).join("/")}`,
    {headers:ghHeaders(token)}
  );
  return r.ok;
}

async function listTemplateFolderFiles(owner, repo, folder, token) {
  const repoInfo=await ghJson(
    `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}`,
    {headers:ghHeaders(token)}
  );
  const tree=await ghJson(
    `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/git/trees/${encodeURIComponent(repoInfo.default_branch)}?recursive=1`,
    {headers:ghHeaders(token)}
  );
...
async function copyTemplateFolderToRepo({templateOwner,templateRepo,folder,targetOwner,targetRepo,token}) {
  const files=await listTemplateFolderFiles(templateOwner,templateRepo,folder,token);
  if(!files.length) return 0;

  for(const filePath of files) {
    const source=await ghJson(
      `https://api.github.com/repos/${encodeURIComponent(templateOwner)}/${encodeURIComponent(templateRepo)}/contents/${filePath.split("/").map(encodeURIComponent).join("/")}`,
      {headers:ghHeaders(token)}
    );
    if(source.encoding!=="base64" || !source.content) continue;

    const existing=await fetch(
      `https://api.github.com/repos/${encodeURIComponent(targetOwner)}/${encodeURIComponent(targetRepo)}/contents/${filePath.split("/").map(encodeURIComponent).join("/")}`,
      {headers:ghHeaders(token)}
    );
    if(existing.ok) continue; // Älä koskaan ylikirjoita opiskelijan jo olemassa olevaa tiedostoa.
    ...
    await ghJson(
      `https://api.github.com/repos/${encodeURIComponent(targetOwner)}/${encodeURIComponent(targetRepo)}/contents/${filePath.split("/").map(encodeURIComponent).join("/")}`,
      {
        ...
        body:JSON.stringify({
          message:`Julkaise ${folder}`,
          content:String(source.content).replace(/\n/g,""),
          branch:undefined
        })
      }
    );
...
async function createBlankCourseRepo({org,repoName,course,student,isPrivate,token}) {
  return ghJson(
    `https://api.github.com/orgs/${encodeURIComponent(org)}/repos`,
    {
      ...
      body:JSON.stringify({
        name:repoName,
        description:`${course.name} – ${student.name||student.id}`,
...
function generatedAssignmentReadme(course, assignment, folder) {
  const due = assignment.dueAt
    ? `\n## Määräaika\n${new Date(assignment.dueAt).toLocaleString("fi-FI")}\n`
    : "";
  return `# ${assignment.name || "Tehtävä"}

Kurssi: ${course.name || ""}

${assignment.instructions || "Tehtävänanto annetaan Koodioppaassa."}
${due}
## Työskentely

Tee tämän tehtävän tiedostot tähän kansioon:
...
\`\`\`bash
git add .
git commit -m "${String(assignment.name||"Tehtävä").replace(/"/g,"'")} valmis"
git push
\`\`\`
`;
}

async function createGeneratedAssignmentFolder({course,assignment,folder,targetOwner,targetRepo,token}) {
  const readmePath=`${folder.replace(/^\/+|\/+$/g,"")}/README.md`;
  const exists=await fetch(
    `https://api.github.com/repos/${encodeURIComponent(targetOwner)}/${encodeURIComponent(targetRepo)}/contents/${readmePath.split("/").map(encodeURIComponent).join("/")}`,
    {headers:ghHeaders(token)}
  );
  if(exists.ok) return {created:false,files:0};

  const text=generatedAssignmentReadme(course,assignment,folder);
  await ghJson(
    `https://api.github.com/repos/${encodeURIComponent(targetOwner)}/${encodeURIComponent(targetRepo)}/contents/${readmePath.split("/").map(encodeURIComponent).join("/")}`,
    {
      method:"PUT",
      headers:{...ghHeaders(token),"Content-Type":"application/json"},
      body:JSON.stringify({
        message:`Julkaise ${assignment.name||folder}`,
        content:Buffer.from(text,"utf8").toString("base64")
      })
    }
  );
  return {created:true,files:1};
}

async function publishAssignmentFolder({course,assignment,folder,targetOwner,targetRepo,token}) {
  if(course.templateRepoUrl) {
    try {
      const {owner,repo}=parseGithubRepoUrl(course.templateRepoUrl);
      const copied=await copyTemplateFolderToRepo({
        templateOwner:owner,
        templateRepo:repo,
        folder,
        targetOwner,
      ...
      if(copied>0) return {mode:"template-folder",files:copied};
    } catch(error) {
      console.warn(`Template-kansion kopiointi ohitettiin (${folder}): ${error.message}`);
    }
  }

  const generated=await createGeneratedAssignmentFolder({
    course,assignment,folder,targetOwner,targetRepo,token
  });
  return {mode:"generated",files:generated.files};
}


async function verifyRepository(owner, repo, token) {
  ...
  if(!response.ok) {
    ...
    return {exists:false,status:response.status,message};
  }
...
async function verifyAssignmentFolder(owner, repo, folder, token) {
  const response=await fetch(
    `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${folder.split("/").map(encodeURIComponent).join("/")}`,
    {headers:ghHeaders(token)}
  );
...
async function verifyCollaborator(owner, repo, username, token) {
  const response=await fetch(
  ...
  return {
    ok:true,
    permission:data.permission||data.user?.permissions||""
  };
}

async function ensureCourseRepo({course,student,org,token,isPrivate}) {
  const repoName=courseRepoName(course,student);
  const url=`https://github.com/${org}/${repoName}`;
  const existing=await fetch(
    `https://api.github.com/repos/${encodeURIComponent(org)}/${encodeURIComponent(repoName)}`,
    {headers:ghHeaders(token)}
  );

  if(existing.ok) {
    return {repoName,repoUrl:url,created:false,creationMode:"existing"};
  }

  await createBlankCourseRepo({org,repoName,course,student,isPrivate,token});
  return {repoName,repoUrl:url,created:true,creationMode:"blank"};
}
...
app.get("/api/student/workflow-status", requireStudent, async (req, res) => {
  try {
    ...
    const deadline=deadlineInfo(assignment.dueAt);
    const late=submissionLateInfo(submission,assignment);
...
app.put("/api/teacher/course-template", requireTeacher, (req,res)=>{
  try{
    const courseId=String(req.body?.courseId||"");
    const templateRepoUrl=String(req.body?.templateRepoUrl||"").trim();
    if(!courseId) return res.status(400).json({error:"Kurssi puuttuu."});
    if(!templateRepoUrl) return res.status(400).json({error:"Template repository puuttuu."});
    parseGithubRepoUrl(templateRepoUrl);

    const data=readJson(FILES.courses,{courses:[]});
    const course=data.courses.find(c=>c.id===courseId);
    if(!course) return res.status(404).json({error:"Kurssia ei löytynyt."});

    course.templateRepoUrl=templateRepoUrl;
    writeJson(FILES.courses,data);

    res.json({ok:true,courseId,templateRepoUrl});
  }catch(error){
...
app.post("/api/distribute/preview", requireTeacher, (req, res) => {
  try {
    ...
    const legacyTemplate=(course.assignments||[]).find(a=>a.templateRepoUrl)?.templateRepoUrl || "";
    if(!course.templateRepoUrl && legacyTemplate) course.templateRepoUrl=legacyTemplate;
    if (course.templateRepoUrl) parseGithubRepoUrl(course.templateRepoUrl);
    if (!cfg.githubOrg) return res.status(400).json({ error: "Aseta ensin GitHub-organisaatio." });

    const folder=assignmentFolder(course,assignment);
    const rows=(group.students||[]).map(student=>({
      studentId:student.id,
      studentName:student.name,
      githubUsername:student.githubUsername||"",
      repoName:courseRepoName(course,student),
      repoUrl:courseRepoUrl(course,student),
      folder,
      valid:Boolean(student.githubUsername)
    }));

    res.json({
      course:{id:course.id,name:course.name,templateRepoUrl:course.templateRepoUrl||"",templateOptional:true},
      assignment:{id:assignment.id,name:assignment.name,folder},
      group:{id:group.id,name:group.name},
      private:course.privateRepo ?? cfg.defaultRepoPrivate,
      rows
    });
  } catch(error) {
    res.status(400).json({error:error.message});
...
async function publishAssignmentForStudent({course,assignment,student,org,token,isPrivate}) {
  const folder=assignmentFolder(course,assignment);
  ...
  let repoName=courseRepoName(course,student);
  let repoUrl=`https://github.com/${org}/${repoName}`;

  try {
    const repo=await ensureCourseRepo({course,student,org,token,isPrivate});
    repoName=repo.repoName;
    repoUrl=repo.repoUrl;

    let repoCheck=await verifyRepository(org,repoName,token);
    if(!repoCheck.exists) {
      await new Promise(resolve=>setTimeout(resolve,800));
      repoCheck=await verifyRepository(org,repoName,token);
    }
    if(!repoCheck.exists) {
      return {
        student:student.name||student.id,
        githubUsername:student.githubUsername,
        repoName,repoUrl,folder,
        status:"repo_verification_failed",
        verified:false,repoExists:false,folderExists:false,collaboratorOk:false,
        error:`Repositoryn luontia ei voitu varmistaa GitHubista (${repoCheck.status}${repoCheck.message?": "+repoCheck.message:""}).`
      };
    }

    const collaboratorResponse=await fetch(
      `https://api.github.com/repos/${encodeURIComponent(org)}/${encodeURIComponent(repoName)}/collaborators/${encodeURIComponent(student.githubUsername)}`,
      {
        method:"PUT",
        headers:{...ghHeaders(token),"Content-Type":"application/json"},
    ...
    if(!collaboratorResponse.ok) {
      try { inviteMessage=(await collaboratorResponse.json()).message||""; } catch {}
    }

    let folderCheck=await verifyAssignmentFolder(org,repoName,folder,token);
    let publication={mode:"existing",files:0};

    if(!folderCheck.exists) {
      publication=await publishAssignmentFolder({
        course,assignment,folder,
        targetOwner:org,targetRepo:repoName,token
      });
      folderCheck=await verifyAssignmentFolder(org,repoName,folder,token);
      if(!folderCheck.exists) {
    ...
    let permissionCheck={ok:false,status:0,message:""};
    try {
      permissionCheck=await verifyCollaborator(org,repoName,student.githubUsername,token);
    } catch {}

    const repoExists=repoCheck.exists;
    const folderExists=folderCheck.exists;
    const collaboratorAccepted=permissionCheck.ok;
    const inviteCreated=collaboratorResponse.ok;
    ...
    if(!repoExists) {
      status="repo_verification_failed";
      error="Repositorya ei löytynyt GitHubista jälkitarkistuksessa.";
    } else if(!folderExists) {
      status="assignment_verification_failed";
      error="Tehtäväkansiota ei löytynyt GitHubista julkaisun jälkitarkistuksessa.";
    } else if(!inviteCreated) {
      status="invite_failed";
      error=inviteMessage || "Opiskelijan GitHub-kutsua ei voitu luoda.";
    } else if(!collaboratorAccepted) {
      status="invite_pending";
    } else if(repo.created) {
      status="verified_course_repo_and_assignment";
    } else {
      status="verified_assignment_published";
    }
    ...
    return {
      ...
      githubUsername:student.githubUsername,
      repoName,
      repoUrl:repoCheck.htmlUrl||repoUrl,
      folder,
      status,
      verified:repoExists && folderExists,
      repoExists,
      folderExists,
      collaboratorOk:collaboratorAccepted,
      invitationCreated:inviteCreated,
      repoCreated:repo.created,
      creationMode:repo.creationMode,
      publicationMode:publication.mode,
      filesCopied:publication.files,
      error
    };
  } catch(error) {
    let repoCheck={exists:false};
    let folderCheck={exists:false};
    try {
      repoCheck=await verifyRepository(org,repoName,token);
      if(repoCheck.exists) folderCheck=await verifyAssignmentFolder(org,repoName,folder,token);
    } catch {}
    ...
    return {
      ...
      status:"failed",
      verified:false,
      repoExists:Boolean(repoCheck.exists),
      folderExists:Boolean(folderCheck.exists),
...
app.post("/api/distribute/batch-preview", requireTeacher, (req,res)=>{
  try {
    const {courseId,assignmentIds,groupId}=req.body||{};
    const courses=readJson(FILES.courses,{courses:[]}).courses||[];
    const course=courses.find(c=>c.id===String(courseId||""));
    const group=findGroup(groupId);
    if(!course||!group) return res.status(404).json({error:"Kurssia tai ryhmää ei löytynyt."});

    const ids=new Set(Array.isArray(assignmentIds)?assignmentIds:[]);
    const assignments=(course.assignments||[]).filter(a=>ids.has(a.id));
    if(!assignments.length) return res.status(400).json({error:"Valitse vähintään yksi tehtävä."});

    const cfg=settings();
    if(!cfg.githubOrg) return res.status(400).json({error:"GitHub-organisaatio puuttuu."});
    ...
    res.json({
      ...
      assignments:assignments.map(a=>({
        id:a.id,name:a.name,folder:assignmentFolder(course,a),published:a.published!==false,dueAt:a.dueAt||""
      })),
      rows,
      private:course.privateRepo ?? cfg.defaultRepoPrivate
    });
...
app.post("/api/distribute/batch-run", requireTeacher, async (req,res)=>{
  try {
    ...
    const isPrivate=course.privateRepo ?? cfg.defaultRepoPrivate;
    const results=[];
    ...
    for(const student of group.students||[]) {
      const studentResult={
        student:student.name||student.id,
        githubUsername:student.githubUsername||"",
        repository:courseRepoUrl(course,student),
        assignments:[]
      };

      for(const assignment of assignments) {
        const result=await publishAssignmentForStudent({
          course,assignment,student,org:cfg.githubOrg,token,isPrivate
        });
        studentResult.repository=result.repoUrl||studentResult.repository;
        studentResult.assignments.push({
    ...
    const summary={
      ...
      successful:results.reduce((sum,sr)=>sum+sr.assignments.filter(a=>a.verified).length,0),
      failed:results.reduce((sum,sr)=>sum+sr.assignments.filter(a=>!a.verified).length,0)
    };
...
app.post("/api/distribute/run", requireTeacher, async (req,res)=>{
  try{
    ...
    for(const student of group.students||[]) {
      ...
      let repoName=courseRepoName(course,student);
      let repoUrl=`https://github.com/${cfg.githubOrg}/${repoName}`;

      try{
        // 1. Varmista/luo kurssirepository
        const repo=await ensureCourseRepo({
          course,student,org:cfg.githubOrg,token,isPrivate
        });
        repoName=repo.repoName;
        repoUrl=repo.repoUrl;

        // GitHub voi olla hetken eventual consistency -tilassa; tarkista kahdesti lyhyellä odotuksella.
        let repoCheck=await verifyRepository(cfg.githubOrg,repoName,token);
        if(!repoCheck.exists) {
          await new Promise(resolve=>setTimeout(resolve,800));
          repoCheck=await verifyRepository(cfg.githubOrg,repoName,token);
        }
        ...
        // 2. Lisää opiskelijalle push-oikeus
        const collaboratorResponse=await fetch(
        ...
        // 3. Julkaise tehtäväkansio
        let folderCheck=await verifyAssignmentFolder(cfg.githubOrg,repoName,folder,token);
        let publication={mode:"existing",files:0};

        if(!folderCheck.exists) {
          publication=await publishAssignmentFolder({
            course,assignment,folder,
            targetOwner:cfg.githubOrg,targetRepo:repoName,token
          });

          folderCheck=await verifyAssignmentFolder(cfg.githubOrg,repoName,folder,token);
          if(!folderCheck.exists) {
            await new Promise(resolve=>setTimeout(resolve,600));
            folderCheck=await verifyAssignmentFolder(cfg.githubOrg,repoName,folder,token);
          }
        ...
        // 4. Tarkista käyttöoikeus, mutta huomioi että invitation voi vielä olla pending.
        let permissionCheck={ok:false,status:0,message:""};
        try {
          permissionCheck=await verifyCollaborator(
            cfg.githubOrg,repoName,student.githubUsername,token
          );
        } catch {}
        ...
        // Viimeinenkin poikkeus tarkistetaan GitHubista ennen tuloksen näyttämistä.
        let repoCheck={exists:false};
        let folderCheck={exists:false};
        try{
          repoCheck=await verifyRepository(cfg.githubOrg,repoName,token);
    ...
    log.distributions.push({
      id:crypto.randomUUID(),
      timestamp:new Date().toISOString(),
      model:"course-repository-v3-verified",
      teacherGithub:req.session.teacherGithub.login,
      organization:cfg.githubOrg,
      courseId,courseName:course.name,
...
app.get("/api/teacher/submissions", requireTeacher, async (req, res) => {
  ...
  for (const item of items) {
    const { assignment } = findCourseAssignment(item.courseId,item.assignmentId);
    const late=submissionLateInfo(item,assignment);
    item.dueAt=assignment?.dueAt||"";
    item.late=late.late;
    item.minutesLate=late.minutesLate;
  }
...
app.get("/api/student/progress", requireStudent, (req,res)=>{
  ...
  for(const course of ws.courses||[]){
    for(const assignment of course.assignments||[]){
      ...
      const deadline=deadlineInfo(assignment.dueAt);
      const late=submissionLateInfo(latest,assignment);
      rows.push({
        ...
        overdue:deadline.overdue && !latest,
        minutesRemaining:deadline.minutesRemaining,
        late:late.late,
...
function deadlineInfo(dueAt, reference=new Date()) {
  if (!dueAt) return { hasDeadline:false, overdue:false, dueAt:null, minutesRemaining:null };
  const due=new Date(dueAt);
  if (Number.isNaN(due.getTime())) return { hasDeadline:false, overdue:false, dueAt:null, minutesRemaining:null };
  const diff=due.getTime()-reference.getTime();
  return { hasDeadline:true, overdue:diff<0, dueAt:due.toISOString(), minutesRemaining:Math.round(diff/60000) };
}

function submissionLateInfo(submission, assignment) {
  if (!submission || !assignment?.dueAt) return { late:false, minutesLate:0 };
  const due=new Date(assignment.dueAt), submitted=new Date(submission.submittedAt);
  if (Number.isNaN(due.getTime()) || Number.isNaN(submitted.getTime())) return { late:false, minutesLate:0 };
  const diff=submitted.getTime()-due.getTime();
  return { late:diff>0, minutesLate:Math.max(0,Math.round(diff/60000)) };
}
...
app.get("/api/teacher/dashboard", requireTeacher, (req,res)=>{
  ...
  for(const group of groups){
    for(const student of group.students||[]){
      ...
      let approved=0,pending=0,changes=0,notSubmitted=0,overdue=0;
      const percents=[];
      ...
      for(const pair of assignments){
        ...
        if(!latest) {
          notSubmitted++;
          if(deadlineInfo(pair.assignment.dueAt).overdue) overdue++;
        }
      ...
      const attention=changes>0 || overdue>0 || highHelp>=3 || (total>=2 && notSubmitted>=Math.ceil(total/2)) || (average!=null && average<60);

      students.push({
        studentId:student.id,studentName:student.name,githubUsername:student.githubUsername,
        groupId:group.id,groupName:group.name,total,approved,pending,changes,notSubmitted,overdue,
  ...
  res.json({
    summary:{
      ...
      overdue:students.reduce((a,x)=>a+x.overdue,0)
    },
    groups:groupRows,
...
/* KURSSIKOHTAINEN REPOSITORYMALLI */
app.get("/api/teacher/course-repository-plan/:courseId", requireTeacher, (req,res)=>{
  const courses=readJson(FILES.courses,{courses:[]}).courses||[];
  const groups=readJson(FILES.groups,{groups:[]}).groups||[];
  const course=courses.find(c=>c.id===String(req.params.courseId||""));
  if(!course) return res.status(404).json({error:"Kurssia ei löytynyt."});
  const selectedGroups=groups.filter(g=>(course.groupIds||[]).includes(g.id));
  const students=selectedGroups.flatMap(g=>(g.students||[]).map(st=>({...st,groupName:g.name})));
  res.json({
    ok:true,
    course:{id:course.id,name:course.name,repoPrefix:course.repoPrefix||slug(course.name),templateRepoUrl:course.templateRepoUrl||"",privateRepo:course.privateRepo!==false},
    assignments:(course.assignments||[]).map((a,i)=>({
      id:a.id,name:a.name,
      folder:a.folder||`tehtava-${String(i+1).padStart(2,"0")}-${slug(a.name||"tehtava")}`,
      published:a.published!==false,dueAt:a.dueAt||""
    })),
    students:students.map(st=>({
      name:st.name,githubUsername:st.githubUsername||"",
      repository:`${course.repoPrefix||slug(course.name)}-${st.githubUsername||"github-tunnus"}`
    }))
  });
});
...
app.listen(PORT, () => {
  console.log(`Koodiopas v32.1: ${BASE_URL}`);
