...
setupCopyButton("copyStartClone","startCloneCommand");
setupCopyButton("copyStartPull","startPullCommand");
setupCopyButton("copyStartCd","startCdCommand");

function renderStartWork(a){
  ...
  card.hidden=false;
  const repoName=String(a.repoUrl).replace(/\/+$/,"").split("/").pop()||"kurssirepo";
  const folder=a.folder||"";

  $("startWorkTitle").textContent=`${repoName}`;
  $("startWorkText").textContent=
    "Kurssi kloonataan vain kerran. Uudet tehtävät tulevat saman repositoryn alle omiin kansioihinsa.";

  $("startCloneCommand").textContent=`git clone ${a.repoUrl}.git`;
  $("startPullCommand").textContent="git pull";
  $("startCdCommand").textContent=folder ? `cd ${folder}` : "cd tehtava-kansio";
}
...
async function loadWorkflowStatus() {
  ...
  $("gitWorkflowTitle").textContent = "✅ Päivittäinen Git-työskentely";
  ...
  const commands = (d.recommendedCommands || []).filter(cmd => !String(cmd).startsWith("git clone "));
...
function updateStudentAssignment() {
  ...
  $("assignmentInfo").innerHTML = `<strong>${esc(a.name)}</strong><p>${esc(a.instructions || "")}</p>${a.folder?`<p class="muted">Tehtäväkansio: <code>${esc(a.folder)}</code></p>`:""}`;
...
async function loadCourses() {
  ...
  refreshCourseTaskFilters();
  applyCourseTaskFilters();
...
function refreshCourseTaskFilters(){
  const modules=[...new Set(
    (coursesData?.courses||[]).flatMap(c=>(c.assignments||[]).map(a=>(a.module||"").trim()).filter(Boolean))
  )].sort((a,b)=>a.localeCompare(b,"fi"));
  const current=$("courseModuleFilter")?.value||"";
  if($("courseModuleFilter")){
    $("courseModuleFilter").innerHTML='<option value="">Kaikki moduulit</option>'+
      modules.map(m=>`<option value="${esc(m)}">${esc(m)}</option>`).join("");
    if(modules.includes(current)) $("courseModuleFilter").value=current;
  }
}

function applyCourseTaskFilters(){
  const q=($("courseTaskSearch")?.value||"").trim().toLowerCase();
  const module=($("courseModuleFilter")?.value||"");
  const pub=($("coursePublishFilter")?.value||"");

  $$(".assignment").forEach(box=>{
    const name=(box.querySelector(".aName")?.value||"").toLowerCase();
    const folder=(box.querySelector(".aFolder")?.value||"").toLowerCase();
    const mod=(box.querySelector(".aModule")?.value||"");
    const published=Boolean(box.querySelector(".aPublished")?.checked);
    const matchesText=!q || name.includes(q) || folder.includes(q) || mod.toLowerCase().includes(q);
    const matchesModule=!module || mod===module;
    const matchesPublish=!pub || (pub==="published" && published) || (pub==="draft" && !published);
    box.hidden=!(matchesText && matchesModule && matchesPublish);
  });
}

if($("courseTaskSearch")) $("courseTaskSearch").oninput=applyCourseTaskFilters;
if($("courseModuleFilter")) $("courseModuleFilter").onchange=applyCourseTaskFilters;
if($("coursePublishFilter")) $("coursePublishFilter").onchange=applyCourseTaskFilters;
...
function addCourseCard(course = { id:uid(), name:"", groupIds:[], assignments:[] }) {
  ...
  card.innerHTML = `
    <div class="between">
      ...
      <div class="courseRepoSettings">
        ...
        <label>Kurssin template repository (valinnainen)</label><input class="cTemplate" placeholder="https://github.com/organisaatio/html-kurssi-template">
        <small class="muted">Template on valinnainen tehtävätiedostojen lähde. Kurssirepo luodaan aina erikseen, joten template ei koskaan tuo opiskelijalle julkaisemattomia tehtäviä.</small>
  ...
  card.querySelector(".cTemplate").value = course.templateRepoUrl || legacyTemplate || "";
  if (!course.templateRepoUrl && legacyTemplate) {
    card.querySelector(".cTemplate").title = "Template siirretty vanhalta tehtävältä kurssitasolle. Tallenna kurssit vahvistaaksesi muutoksen.";
  }
...
function addAssignmentRow(container, a = {}) {
  ...
  box.innerHTML = `
    <div class="assignmentGrid">
      ...
      <div><label>Moduuli / osio</label><input class="aModule" placeholder="Esim. HTML perusteet"></div>
  ...
  box.querySelector(".aModule").value = a.module || "";
  ...
  box.querySelector(".aName").addEventListener("input", applyCourseTaskFilters);
  box.querySelector(".aFolder").addEventListener("input", applyCourseTaskFilters);
  box.querySelector(".aModule").addEventListener("input", () => {
    refreshCourseTaskFilters();
    applyCourseTaskFilters();
  });
  box.querySelector(".aPublished").addEventListener("change", applyCourseTaskFilters);
}
  ...
  try {
    ...
    assignments: [...card.querySelectorAll(".assignment")].map(box => {
      ...
      return {
        ...
        module: box.querySelector(".aModule").value.trim(),
...
async function prepareDistribution() {
  ...
  $("missingTemplateCard").hidden = true;
  $("distributionTemplateStatus").textContent = "";
}
...
$("preview").onclick = async () => {
  ...
  if (!r.ok) {
    ...
    return alert(d.error || "Esikatselu epäonnistui.");
  }
  ...
  $("previewInfo").textContent =
    `${d.course.name} • ${d.assignment.name} • ${d.group.name} • ${d.private ? "yksityiset" : "julkiset"} kurssirepositoryt • ` +
    (d.course.templateRepoUrl ? "template toimii valinnaisena tehtävätiedostojen lähteenä" : "tehtäväkansio luodaan Koodioppaasta");

  $("previewTable").innerHTML = `
    <table><thead><tr><th>Opiskelija</th><th>GitHub</th><th>Kurssirepository</th><th>Tehtäväkansio</th><th>Valmis</th></tr></thead>
    <tbody>${d.rows.map(x => `
      <tr><td>${esc(x.studentName)}</td><td>${x.githubUsername ? "@" + esc(x.githubUsername) : "—"}</td>
      <td><code>${esc(x.repoUrl)}</code></td><td><code>${esc(x.folder||"")}</code></td><td>${x.valid ? "✅" : "❌ GitHub-tunnus puuttuu"}</td></tr>`).join("")}
    </tbody></table>`;
};


$("saveDistributionTemplate").onclick=async()=>{
  const courseId=$("courseSelect").value;
  const templateRepoUrl=$("distributionTemplateUrl").value.trim();

  if(!courseId){
    $("distributionTemplateStatus").textContent="Valitse ensin kurssi.";
    return;
  }
  if(!templateRepoUrl){
    $("distributionTemplateStatus").textContent="Anna Template repositoryn GitHub-osoite.";
    return;
  }

  $("distributionTemplateStatus").textContent="Tallennetaan…";

  const r=await fetch("/api/teacher/course-template",{
    method:"PUT",
    headers:{"Content-Type":"application/json"},
    body:JSON.stringify({courseId,templateRepoUrl})
  });
  const d=await r.json();

  if(!r.ok){
    $("distributionTemplateStatus").textContent=d.error||"Templatea ei voitu tallentaa.";
    return;
  }

  $("distributionTemplateStatus").textContent="Tallennettu ✓ Jatketaan esikatseluun…";
  await prepareDistribution();

  // restore selected course and re-render choices
  $("courseSelect").value=courseId;
  renderDistributionChoices();

  // user still needs assignment/group re-selection if their DOM reset;
  // keep clear instruction rather than guessing.
  $("distributionTemplateStatus").textContent="";
  $("missingTemplateCard").hidden=true;
  alert("Template tallennettiin kurssille. Valitse tehtävä ja ryhmä uudelleen ja paina Esikatsele jako.");
};


function renderBatchTaskChoices(){
  const course=coursesData?.courses?.find(c=>c.id===$("courseSelect").value);
  const list=$("batchTaskList");
  $("batchPreview").hidden=true;
  $("batchResults").hidden=true;
  $("runBatchDistribution").hidden=true;
  batchPreviewData=null;
  if($("batchPublishStatus")) $("batchPublishStatus").textContent="";

  if(!course){
    list.innerHTML='<p class="muted">Valitse ensin kurssi.</p>';
    return;
  }
  ...
  for(const a of assignments){
    const key=(a.module||"Muut tehtävät").trim()||"Muut tehtävät";
    if(!groups.has(key)) groups.set(key,[]);
    groups.get(key).push(a);
  }
  list.innerHTML=assignments.length ? [...groups.entries()].map(([module,items])=>`
    <div class="batchModule">
      <div class="batchModuleHeader">
        <strong>${esc(module)}</strong>
        <button type="button" class="secondary selectBatchModule" data-module="${esc(module)}">Valitse moduuli</button>
      </div>
      <div class="batchModuleTasks">
        ${items.map(a=>`
          <label class="batchTaskRow" data-module="${esc(module)}">
            <input type="checkbox" class="batchTaskCheck" value="${esc(a.id)}">
            <span><strong>${esc(a.name)}</strong><small>${esc(a.folder||"")}${a.dueAt?` • ${new Date(a.dueAt).toLocaleString("fi-FI")}`:""}</small></span>
          </label>`).join("")}
      </div>
    </div>`).join("") : '<p class="muted">Kurssilla ei ole tehtäviä.</p>';

  $$(".selectBatchModule").forEach(btn=>{
    btn.onclick=()=>{
      const module=btn.dataset.module;
      const checks=[...document.querySelectorAll(`.batchTaskRow[data-module="${CSS.escape(module)}"] .batchTaskCheck`)];
      const all=checks.length && checks.every(c=>c.checked);
      checks.forEach(c=>c.checked=!all);
      btn.textContent=all?"Valitse moduuli":"Poista moduulin valinnat";
    };
  });
}

$("selectAllBatchTasks").onclick=()=>{
  const checks=$$(".batchTaskCheck");
  const allSelected=checks.length && checks.every(c=>c.checked);
  checks.forEach(c=>c.checked=!allSelected);
  $("selectAllBatchTasks").textContent=allSelected?"Valitse kaikki":"Poista valinnat";
};

let batchPreviewData=null;

function selectedBatchAssignmentIds(){
  return $$(".batchTaskCheck").filter(c=>c.checked).map(c=>c.value);
}

$("previewBatchDistribution").onclick=async()=>{
  try{
    const courseId=$("courseSelect").value;
    const groupId=$("groupSelect").value;
    const assignmentIds=selectedBatchAssignmentIds();

    batchPreviewData=null;
    $("runBatchDistribution").hidden=true;
    $("batchPublishStatus").textContent="";

    if(!courseId||!groupId){
      $("batchPublishStatus").textContent="Valitse kurssi ja ryhmä.";
      return;
    }
    if(!assignmentIds.length){
      $("batchPublishStatus").textContent="Valitse vähintään yksi tehtävä.";
      return;
    }

    $("previewBatchDistribution").disabled=true;
    $("previewBatchDistribution").textContent="Tarkistetaan…";

    const r=await fetch("/api/distribute/batch-preview",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({courseId,groupId,assignmentIds})
    });

    const raw=await r.text();
    let d={};
    try{ d=raw?JSON.parse(raw):{}; }catch{
      throw new Error(`Palvelin palautti virheellisen vastauksen (${r.status}).`);
    }

    if(!r.ok){
      $("batchPublishStatus").textContent=d.error||"Massajaon esikatselu epäonnistui.";
      return;
    }

    // Lukitse juuri esikatseltu valinta. Julkaisu käyttää tätä snapshotia,
    // eikä lue lomaketta uudelleen painallushetkellä.
    batchPreviewData={
      courseId:d.course.id,
      groupId:d.group.id,
      assignmentIds:d.assignments.map(a=>a.id),
    ...
    $("batchPreview").innerHTML=`
      <h3>Massajaon esikatselu</h3>
      <p><strong>${esc(d.course.name)}</strong> • ${esc(d.group.name)} • ${d.assignments.length} tehtävää • ${d.rows.length} opiskelijaa</p>
      <div class="batchAssignmentSummary">
        ${d.assignments.map(a=>`<span><strong>${esc(a.name)}</strong><code>${esc(a.folder)}</code></span>`).join("")}
      </div>
      <p class="muted">Yhteensä enintään ${d.assignments.length*d.rows.length} tehtäväjulkaisua. Kurssirepo luodaan opiskelijalle vain, jos sitä ei vielä ole.</p>`;

    $("runBatchDistribution").hidden=false;
    $("batchPublishStatus").textContent="Esikatselu valmis. Voit nyt julkaista valitut tehtävät.";
  }catch(error){
    console.error("Massajaon esikatselu:",error);
    $("batchPublishStatus").textContent=`Esikatselu epäonnistui: ${error.message}`;
  }finally{
    $("previewBatchDistribution").disabled=false;
    $("previewBatchDistribution").textContent="Esikatsele massajako";
  }
};

$("runBatchDistribution").onclick=async()=>{
  if(!batchPreviewData){
    $("batchPublishStatus").textContent="Tee ensin massajaon esikatselu.";
    return;
  }

  const {courseId,groupId,assignmentIds,courseName,groupName}=batchPreviewData;

  if(!assignmentIds?.length){
    $("batchPublishStatus").textContent="Esikatselussa ei ole julkaistavia tehtäviä.";
    return;
  }

  const ok=confirm(
    `Julkaistaanko ${assignmentIds.length} tehtävää ryhmälle ${groupName}? `+
    `Olemassa olevia tehtäväkansioita ei ylikirjoiteta.`
  );
  if(!ok){
    $("batchPublishStatus").textContent="Julkaisu peruutettu.";
    return;
  }

  const button=$("runBatchDistribution");

  try{
    button.disabled=true;
    button.textContent="Julkaistaan…";
    $("previewBatchDistribution").disabled=true;
    $("batchPublishStatus").textContent=
      `Julkaistaan ${assignmentIds.length} tehtävää ryhmälle ${groupName}. Tämä voi kestää hetken…`;

    const r=await fetch("/api/distribute/batch-run",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({courseId,groupId,assignmentIds})
    });
    ...
    if(!r.ok){
      throw new Error(d.error||`Massajako epäonnistui (${r.status}).`);
    }

    $("batchResults").hidden=false;
    $("batchResults").innerHTML=`
      <h3>Massajaon tulos</h3>
      <div class="stats compactStats">
        <div class="stat"><span>Opiskelijoita</span><strong>${d.summary.students}</strong></div>
        <div class="stat"><span>Tehtäviä</span><strong>${d.summary.assignments}</strong></div>
        <div class="stat"><span>Onnistui</span><strong>${d.summary.successful}</strong></div>
        <div class="stat"><span>Ei varmistunut</span><strong>${d.summary.failed}</strong></div>
      </div>
      <div class="batchStudentResults">
        ${d.results.map(sr=>`
          <details>
            <summary><strong>${esc(sr.student)}</strong> • ${sr.assignments.filter(a=>a.verified).length}/${sr.assignments.length} varmistettu</summary>
            <div class="batchResultRows">
              ${sr.assignments.map(a=>`
                <div class="${a.verified?"batchOk":"batchFail"}">
                  <span><strong>${esc(a.assignmentName)}</strong><code>${esc(a.folder)}</code></span>
                  <span>${statusLabel(a.status)}</span>
                  <span>${esc(a.error||"")}</span>
                </div>`).join("")}
            </div>
          </details>`).join("")}
      </div>`;
    ...
    $("batchPublishStatus").textContent=
      `Valmis: ${d.summary.successful} julkaisua varmistettu, ${d.summary.failed} ei varmistunut.`;

    // Jätä snapshot voimaan, jotta saman jaon voi tarvittaessa yrittää uudelleen
    // vain jos käyttäjä niin haluaa.
  }catch(error){
    console.error("Massajako:",error);
    $("batchPublishStatus").textContent=`Julkaisu epäonnistui: ${error.message}`;
  }finally{
    button.disabled=false;
    button.textContent="Julkaise valitut tehtävät";
    $("previewBatchDistribution").disabled=false;
  }
};
...
$("distribute").onclick = async () => {
  ...
  if (!confirm(`Julkaistaanko tehtävä ${count} opiskelijalle? Uusi kurssirepository luodaan vain opiskelijoille, joilla sitä ei vielä ole.`)) return;
  ...
  $("results").innerHTML = `
    <table><thead><tr><th>Opiskelija</th><th>Tila</th><th>Repository</th><th>Varmistus</th><th>Huomio</th></tr></thead>
...
function statusLabel(status) {
  return status === "verified_course_repo_and_assignment" ? "✅ repo + tehtävä varmistettu" :
    status === "verified_assignment_published" ? "✅ tehtävä varmistettu" :
    status === "invite_pending" ? "📨 tehtävä julkaistu, kutsu odottaa" :
    status === "invite_failed" ? "⚠️ repo + tehtävä löytyvät, kutsu epäonnistui" :
    status === "repo_verification_failed" ? "❌ repositorya ei löydy" :
    status === "assignment_verification_failed" ? "❌ tehtäväkansiota ei löydy" :
    status === "course_repo_created_and_assignment_published" ? "⚠️ vanha tila: ei jälkitarkistusta" :
    status === "assignment_published" ? "⚠️ vanha tila: ei jälkitarkistusta" :
    status === "assignment_exists" ? "ℹ️ tehtävä oli jo repossa" :
    status === "created_invite_failed" ? "⚠️ vanha tila: kutsu epäonnistui" :
    status === "skipped" ? "⏭️ ohitettu" :
    "❌ epäonnistui";
}
