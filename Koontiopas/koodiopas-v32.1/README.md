...
## V30 – Aloita tehtävä -ohjaus

Opiskelijan työtilaan lisättiin uusi **Aloita tehtävä** -kortti.

Kortti erottaa selkeästi kaksi tilannetta:

### Ensimmäinen työskentelykerta tällä koneella

```bash
git clone https://github.com/organisaatio/kurssirepo-opiskelija.git
```

### Kurssirepo on jo koneella

```bash
git pull
```

Tämän jälkeen opiskelija siirtyy valitun tehtävän kansioon:

```bash
cd tehtava-02-otsikot
```

Kurssirepository kloonataan siis vain kerran. Git-työvaiheiden kortista poistettiin turha toinen clone-komento, jotta opiskelijalle ei synny vaikutelmaa, että jokainen tehtävä kloonataan erikseen.


## V31 – tehtävien massajulkaisu

Opettaja voi nyt valita samalla kertaa useita kurssin tehtäviä ja julkaista ne yhdelle ryhmälle.

Massajako:
- käyttää yhtä kurssirepositorya per opiskelija
- luo kurssirepon vain, jos sitä ei vielä ole
- lisää vain puuttuvat tehtäväkansiot
- ei ylikirjoita opiskelijan olemassa olevia tehtävätiedostoja
- tekee jokaiselle tehtäväjulkaisulle GitHub-jälkitarkistuksen
- näyttää opiskelijakohtaisen tuloksen avattavassa listassa

Tämä on tarkoitettu erityisesti kursseille, joilla on kymmeniä tai jopa 100 tehtävää.

## V32 – moduulit, haku ja suodatus

Pitkiä kursseja varten tehtäville voi määrittää moduulin/osion.

Kurssit ja tehtävät -näkymässä tehtäviä voi hakea nimellä, kansiolla tai moduulilla ja suodattaa moduulin tai julkaisutilan mukaan.

Massajulkaisussa tehtävät ryhmitellään moduuleittain ja kokonaisen moduulin voi valita yhdellä painikkeella.

Vanhoille tehtäville `module` täydennetään automaattisesti tyhjäksi.


### V32.1 – massajulkaisun painike korjattu

`Julkaise valitut tehtävät` käyttää nyt aina massajaon esikatselussa lukittua kurssi-, ryhmä- ja tehtävävalintaa.

Aiemmin painike luki valinnat uudelleen lomakkeesta ja saattoi poistua hiljaisesti, jos jokin arvo oli tyhjä tai muuttunut.

Nyt:
- esikatselu tallentaa valinnan snapshotiksi
- julkaisu käyttää snapshotia
- hiljaisia `return`-poistumisia ei enää ole
- käyttöliittymä näyttää aina tilan: tarkistus, julkaisu, valmis tai virhe
- virheellinen palvelinvastaus näkyy käyttäjälle
- painikkeet poistetaan käytöstä julkaisun ajaksi
