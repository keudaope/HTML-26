...
function showTeacherView(view) {
  for (const name of ["dashboard","groups","courses","github","distribute","rubrics","submissions","history","analytics"]) {
...
async function loadMigrationStatus(){
  ...
  target.innerHTML=`<div class="migrationList">${changed.map(x=>{
    const label=x.action==="created"?"luotu":x.action==="updated"?"täydennetty":x.action==="schema-updated"?"rakennetta täydennetty":x.action==="repaired"?"korjattu varmuuskopiosta":"päivitetty";
...
function renderStudentDashboard(){
  ...
  <table><thead><tr><th>Opiskelija</th><th>Ryhmä</th><th>Valmis</th><th>Arvioitavana</th><th>Korjattavana</th><th>Ei palautettu</th><th>Keskiarvo</th><th>Avunpyynnöt</th><th>Tila</th></tr></thead>
  ...
  <td>${esc(x.groupName)}</td><td>${x.approved}/${x.total} (${x.completion} %)</td><td>${x.pending}</td><td>${x.changes}</td><td>${x.notSubmitted}</td>
