...
function analyticsToCsv(events) {
  const rows = [["aika", "ryhma", "opiskelija", "tunniste", "tehtava", "vihjetaso", "teema"]];
  for (const e of events) {
    rows.push([e.timestamp, e.groupName || "", e.studentName || "", e.studentId || "", e.project || "", e.level || 1, e.theme || ""]);
  }
...
app.get("/api/analytics/export.csv", requireTeacher, (req, res) => {
  const data = readAnalytics();
  const filters = {
    groupId: String(req.query.groupId || ""),
    studentId: String(req.query.studentId || ""),
  ...
  const csv = analyticsToCsv(filterEvents(data.events || [], filters));
  res.setHeader("Content-Type", "text/csv; charset=utf-8");
  res.setHeader("Content-Disposition", 'attachment; filename="koodiopas-analytiikka.csv"');
  res.send(csv);
});
