...
$("exportCsv").onclick=()=>{
  window.location="/api/analytics/export.csv?"+currentFilterParams().toString();
};
...
function parseCsv(text){
  ...
  return lines.slice(1).map(line=>{
    const values=parseRow(line),row={};
...
$("importCsv").onclick=async()=>{
  ...
  if(!file){$("csvStatus").textContent="Valitse ensin CSV-tiedosto.";return;}
  const rows=parseCsv(await file.text());
  const groupMap=new Map(groupsData.groups.map(g=>[g.name,g]));
  let added=0,updated=0;
  for(const row of rows){
    const groupName=row.ryhma||row["ryhmä"]||row.group||"";
    const name=row.opiskelija||row.nimi||row.student||"";
    const id=row.tunniste||row.id||"";
    const repoUrl=row.github||row.repository||row.repo||"";
    if(!groupName||(!name&&!id))continue;
    if(!groupMap.has(groupName)){
      const group={id:crypto.randomUUID(),name:groupName,students:[]};
      groupsData.groups.push(group);groupMap.set(groupName,group);
    }
    const group=groupMap.get(groupName);
    const existing=group.students.find(s=>(id&&s.id===id)||(!id&&name&&s.name===name));
    if(existing){existing.name=name||existing.name;existing.repoUrl=repoUrl||existing.repoUrl;updated++;}
    else{group.students.push({id:id||crypto.randomUUID(),name,repoUrl});added++;}
  }
  renderGroupsEditor();
  $("csvStatus").textContent=`CSV luettu. Uusia: ${added}, päivitettyjä: ${updated}. Muista tallentaa ryhmät.`;
};

$("downloadExample").onclick=()=>{
  const csv="\uFEFFryhma;opiskelija;tunniste;github\nOhjelmistokehittäjä 2026;Matti Meikäläinen;matti01;https://github.com/kayttaja/repo\nOhjelmistokehittäjä 2026;Maija Mallikas;maija02;https://github.com/kayttaja/repo2\n";
  const blob=new Blob([csv],{type:"text/csv;charset=utf-8"});
  const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="koodiopas-opiskelijat-esimerkki.csv";a.click();URL.revokeObjectURL(a.href);
};
