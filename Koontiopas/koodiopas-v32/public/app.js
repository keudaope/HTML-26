...
$("previewBatchDistribution").onclick=async()=>{
  ...
  if(!courseId||!groupId){
    alert("Valitse kurssi ja ryhmä.");
    return;
  }
  if(!assignmentIds.length){
    alert("Valitse vähintään yksi tehtävä.");
    return;
  }
  ...
  const d=await r.json();
  if(!r.ok){
    alert(d.error||"Massajaon esikatselu epäonnistui.");
    return;
  }
...
$("runBatchDistribution").onclick=async()=>{
  ...
  const groupId=$("groupSelect").value;
  const assignmentIds=selectedBatchAssignmentIds();
  if(!courseId||!groupId||!assignmentIds.length)return;

  if(!confirm(`Julkaistaanko ${assignmentIds.length} tehtävää valitulle ryhmälle? Olemassa olevia tehtäväkansioita ei ylikirjoiteta.`))return;

  $("runBatchDistribution").disabled=true;
  $("runBatchDistribution").textContent="Julkaistaan…";
  ...
  $("runBatchDistribution").disabled=false;
  $("runBatchDistribution").textContent="Julkaise valitut tehtävät";

  if(!r.ok){
    alert(d.error||"Massajako epäonnistui.");
    return;
  }
