...
app.post("/api/distribute/preview", requireTeacher, (req, res) => {
  try {
    ...
    if (!courseTemplate) {
      return res.status(400).json({ error: "Kurssilta puuttuu Template repository. Lisää se Kurssit ja tehtävät -näkymän Kurssin repository -osioon." });
    }
...
app.post("/api/distribute/run", requireTeacher, async (req,res)=>{
  try{
    ...
    const courseTemplate=course.templateRepoUrl || (course.assignments||[]).find(a=>a.templateRepoUrl)?.templateRepoUrl || "";
    if(!courseTemplate) return res.status(400).json({error:"Kurssilta puuttuu Template repository. Lisää se Kurssit ja tehtävät -näkymän Kurssin repository -osioon."});
    course.templateRepoUrl=courseTemplate;
...
app.listen(PORT, () => {
  console.log(`Koodiopas v29.3: ${BASE_URL}`);
